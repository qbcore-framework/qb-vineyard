...
Zones[1].zone:onPlayerInOut(function(isPointInside)
	...
	if isPointInside then
		...
		if not startVineyard and PlayerJob.name == "vineyard" then
			exports['qb-core']:DrawText(Lang:t("task.start_task"),'left')
			CreateThread(function()
				...
				while Zones[2].isInside do
					if not wineStarted then
						if not loadIngredients then
							exports['qb-core']:DrawText(Lang:t("task.load_ingrediants"),'left')
							...
							if not finishedWine then
								exports['qb-core']:DrawText(Lang:t("task.wine_process"),'left')
								if IsControlJustPressed(0, 38) and not LocalPlayer.state.inv_busy then
									StartWineProcess()
								end
							else
								exports['qb-core']:DrawText(Lang:t("task.get_wine"),'left')
								if IsControlJustPressed(0, 38) and not LocalPlayer.state.inv_busy then
						...
						exports['qb-core']:DrawText(Lang:t("task.countdown",{time=winetimer}),'left')
						Wait(999)
					end
					Wait(1)
				end
		...
		if not startVineyard and PlayerJob.name == "vineyard" then
			CreateThread(function()
				while Zones[3].isInside do
					exports['qb-core']:DrawText(Lang:t("task.make_grape_juice"),'left')
					if IsControlJustPressed(0, 38) and not LocalPlayer.state.inv_busy then
